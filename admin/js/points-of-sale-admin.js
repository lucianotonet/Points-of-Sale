(function( $ ) {
	...
	$(document).ready(function($) {
    	...
    	if( !document.getElementById('pos_locationpicker') ){
    		console.log();
    	}else{
		    ...
		    if( document.getElementById('_pos_longitude').value != "" && document.getElementById('_pos_longitude').value != "" ) {
		    	// OLD VALUES
			    lat = pos_lat.val();
			    lon = pos_long.val();	    
		    }else if( navigator.geolocatio ){
		    	// GPS
			...
			google.maps.event.addListener(searchBox, 'places_changed', function() {
				...
				for (var i = 0, place; place = places[i]; i++) {				 
				    ...
				    //var marker = marker.setPosition( place.geometry.location );
		...
		function reloadAddress(){
	        ...
	        geocoder.geocode({latLng: marker.getPosition()}, function(responses) {
	            if (responses && responses.length > 0) {
	            	...
	            	pos_lat.val( marker.getPosition().lat() );
	            	pos_long.val( marker.getPosition().lng() );
